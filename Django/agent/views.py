...
class AgentIndexView(View):
    """List agents and optionally show a selected agent (conversation) with message area.

    GET: Render list and optional selected conversation.
    POST: Create new agent or initial prompt for selected conversation.
    """
    ...
    def post(self, request, conversation_id=None):
        # Determine selected business context similar to GET
        selected_business = None
        biz_id = request.GET.get("biz")
        if biz_id:
            try:
...
class AgentHomeView(View):
    ...
    def get(self, request):
        ...
        conversations = (
            Agent.objects.filter(business=selected_business)
            if selected_business
            else Agent.objects.order_by("-updated_at")
        )
        return render(
            request,
            "agent/agent.html",
